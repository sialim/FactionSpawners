...
public class FactionUnadminCommand implements SubCommand{
    private final FactionCommand commandManager;
    private final ActionBarMessages util;

    public FactionUnadminCommand(FactionCommand commandManager) {
    ...
    public void execute(CommandSender sender, String[] args) {
        ...
        if (args.length != 1) {
            util.actionBar(p, "/f unadmin <player>");
            return;
        }
        ...
        if (!faction.isAdmin(tUUID)) {
            util.actionBar(p, "player not a faction admin", ChatColor.RED);
            return;
        }
    ...
    public List<String> tabComplete(CommandSender sender, String[] args) {
        ...
        return Bukkit.getOnlinePlayers().stream()
                .filter(t -> faction.admins.contains(t.getUniqueId()))
                .map(Player::getName)
